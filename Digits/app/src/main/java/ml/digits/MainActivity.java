...
public class MainActivity extends Activity  {
    ...
    int previousLoop = 0;
    float volume;
	long timeWhenStopped; // Stores time elapsed in memory when stopping/restarting the chronometer in onPause & onResume
    ...
    protected void onPause() {
		...
		Log.e("timeWhenStopped ", String.valueOf(timeWhenStopped));
		chronometer.stop();
		if (soundEnabled) {
			try {
				if (playingLoop) {
